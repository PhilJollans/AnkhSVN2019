    ...
    partial class OpenDocumentTracker : AnkhService, IAnkhOpenDocumentTracker, IVsRunningDocTableEvents4, IVsRunningDocTableEvents3, IVsRunningDocTableEvents2, IVsRunningDocTableEvents
    {
        ...
        public int OnAfterLastDocumentUnlock(IVsHierarchy pHier, uint itemid, string pszMkDocument, int fClosedWithoutSaving)
        {
            ...
            if (_docMap.TryGetValue(pszMkDocument, out data))
            {
                ...
                // At this point we could check for a temporary file generated by the annotate command
                // and [1] remove the entry from the AnnoateService and [2] delete the temporary file
                // and directory.
                // However, I am reluctant to add a reference from Ankh.Scc to Ankh.UI (even though I
                // think it would be possible without creating a circular reference). I am more inclined
                // to move the AnnotateService to Ankh.Services.
